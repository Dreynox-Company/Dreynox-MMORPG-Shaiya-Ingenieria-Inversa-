...
namespace Dreynox.Mmorpg.LocalData
{
    ...
    public static class LegacyRuntimeSkinnedBuilder
    {
            ...
            LegacyAniFile ani)
        {
            ...
            int boneCount = ani.Bones.Count;

            if (boneCount == 0 ||
                mesh.InverseBindMatrices.Count < boneCount)
            {
            ...
            ValidateMeshForSkeleton(mesh, boneCount);
            ...
            string meshName)
        {
            ...
            {
                // Keep each piece's authored inverse bind pose, not the torso's.
