...
namespace Dreynox.Mmorpg.Gameplay.CameraSystem
{
    ...
    public sealed class ShaiyaThirdPersonCamera : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (!Dreynox.Mmorpg.Interaction.WorldInputGate.IsBlocked && !Dreynox.Mmorpg.UI.NativeUiPrimitives.PointerCaptured && Application.isFocused && Input.GetMouseButton(1))
                look += new Vector2(Input.GetAxisRaw("Mouse X"), Input.GetAxisRaw("Mouse Y"));
            if (!Dreynox.Mmorpg.Interaction.WorldInputGate.IsBlocked && !Dreynox.Mmorpg.UI.NativeUiPrimitives.PointerCaptured && Application.isFocused) zoom += Input.mouseScrollDelta.y;
