...
{
    // C# 5 / .NET Framework 4.8. No shell, network, elevation or original DATA access.
    public static class VerifiedPlayerArchive
    {
        ...
        {
            using (var sha = SHA256.Create()) return Hex(sha.ComputeHash(input));
        }
        ...
        {
            if (input == null || !input.CanRead) throw new ArgumentException("Readable payload required.");
            ...
            using (var archive = new ZipArchive(input, ZipArchiveMode.Read, true))
            {
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    if (unixType == 0xa000 || (entry.ExternalAttributes & 0x400) != 0)
                        throw new InvalidDataException("Links are not allowed in the Player payload.");
                ...
                if (!files.ContainsKey(GameExecutable) || !files.ContainsKey("UnityPlayer.dll") ||
                    !files.ContainsKey(ManifestName) || !HasDataFolder(files))
                    throw new InvalidDataException("This is not a complete Map1 Unity Player.");
                var expected = ReadManifest(files[ManifestName]);
                if (expected.Count != files.Count - 1)
                    throw new InvalidDataException("The Player manifest does not cover the entire payload.");
                foreach (string name in files.Keys)
                    if (name != ManifestName && !expected.ContainsKey(name))
                        throw new InvalidDataException("Unverified Player file: " + name);
                int complete = 0;
                ...
                foreach (var pair in files)
                {
                    ...
                    complete++;
                    if (progress != null) progress((int)(100L * complete / files.Count));
                }
            }
        }
        ...
        {
            foreach (string name in files.Keys)
                if (name.StartsWith("DreynoxMmorpg-Map1_Data/", StringComparison.Ordinal)) return true;
            return false;
        }
